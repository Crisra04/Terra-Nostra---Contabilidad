(function(){
'use strict';
window.APP_VERSION='1.10'; window.$ = window.$ || ((q,d=document)=>d.querySelector(q));
const db=window.db||(window.db={tx:[]}); function save(){} function toast(x){} function toBOB(t){return Number(t.amount||0)*(t.currency==='USD'?(t.fx||6.96):1)}; function uid(){return Math.random().toString(36).slice(2)};

// ===== Core helpers =====
window.APP_VERSION = "1.10";
window.$ = window.$ || ((q,d=document)=>d.querySelector(q));
...
document.getElementById('genCuotasMes')?.addEventListener('click', ()=>{
  ...
  (db.deudas||[]).forEach(d=>{
    ...
    const bancos = (db.accounts.find(a=>a.code==='1.1.1.002')?.code) || '1.1.1.002';
    const prestamo = (db.accounts.find(a=> a.name?.toUpperCase().includes('PRÉSTAMO') || a.name?.toUpperCase().includes('PRESTAMO'))?.code) || '2.1.2.005';
    const inter = (db.accounts.find(a=> a.name?.toUpperCase().includes('INTERESES FINANCIEROS PAGADOS'))?.code) || '5.4.1.003';
